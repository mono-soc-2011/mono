...
namespace Mono.CodeContracts.CodeAnalysis.Core.AST.Builders
{
  public class BodyParser
  {
    ...
    public BodyParser(MethodDefinition method)
    {
      ...
      CoreSystemTypes.ModuleDefinition = this._method.Module;
    }
    ...
    public List<Statement> ParseBlocks()
    {
      ...
      int counter = 0;
      Collection<Instruction> instructions = this._method.Body.Instructions;
    ...
    private int ParseStatement(Collection<Instruction> instructions, int index, List<Statement> result, out bool isNewBlock)
    {
      ...
      while (index < instructions.Count) {
        ...
        switch (opcode) {
          ...
          case Code.Ldc_I4_7:
          case Code.Ldc_I4_8:
            expression = GetLiteral ((opcode - Code.Ldc_I4_0));
            break;
          case Code.Ldc_I8:
            expression = GetLiteral ((Int64) inst.Operand);
            break;
          case Code.Ldc_I4_S:
            expression = GetLiteral ((int) (sbyte) inst.Operand);
            break;
          case Code.Ldc_I4:
            expression = GetLiteral ((int) inst.Operand);
            break;
          case Code.Ldc_R4:
            expression = GetLiteral ((float) inst.Operand);
            break;
          case Code.Ldc_R8:
            expression = GetLiteral ((double) inst.Operand);
            break;
          case Code.Ret:
            statement = new Return (TypeIsVoid (this._method.ReturnType) ? null : PopOperand ());
            isNewBlock = true;
            needToRepeat = false;
            break;
          case Code.Br_S:
          ...
          case Code.Ldobj:
            break;
          case Code.Ldstr:
            expression = GetLiteral (inst.Operand);
            break;
          case Code.Newobj:
        ...
        if (!this.block_map.ContainsKey (offset)) this.evaluation_stack.Push (expression);
        else {
          isNewBlock = true;
          break;
        }
      ...
      if (statement == null) statement = new ExpressionStatement (expression);

      result.Add (statement);
      if (!isNewBlock) isNewBlock = this.block_map.ContainsKey (instructions[index].Offset);
    ...
    private Expression ParseNewObjectCreation(Instruction inst)
    {
      ...
      List<Expression> arguments;
      {
        ...
        for (int index = count - 1; index >= 0; index--)
          expressions[index] = this.PopOperand ();
    ...
    private Method GetMethodFromMethodCallInstruction(Instruction instruction)
    {
      ...
      Method method = new Method (methodReference.Resolve ());
      return method;
    }
    ...
    private Expression GetParameterExpression(int index)
    {
      if (this._method.IsStatic)
        return new Parameter (this._method.Parameters[index]);
      if (index == 0)
        return new Parameter (this._method.Body.ThisParameter);

      return new Parameter (this._method.Parameters[index - 1]);
    }
    ...
    {
      return new Local (this._method.Body.Variables[index]);
    }
    ...
    private Literal GetLiteral(object constant)
    {
    ...
    {
      foreach (Instruction inst in this._method.Body.Instructions) ProcessInstructionForBlockMap(inst);
    }
